...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Also insert if not exists for new keys
...
<div class="flex-between mb-3">
    <div>
        ...
        <p>Set the default structure, types, and marks distribution for generated exams across the university.</p>
    </div>
</div>

<?= $message ?>

<div class="surface-card" style="max-width: 900px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 2fr 2fr 2fr; gap: 1.5rem; align-items: end; margin-bottom: 1.5rem; background: var(--bg-main); padding: 1.5rem; border-radius: var(--radius-sm); border: 1px solid var(--border-color);">
                ...
                <div class="form-group" style="margin: 0;">
                    ...
                    <select name="sec_a_type" class="form-control" required>
                        <option value="objective" <?= ($settings['sec_a_type'] ?? 'objective') === 'objective' ? 'selected' : '' ?>>Objective</option>
                        <option value="short" <?= ($settings['sec_a_type'] ?? '') === 'short' ? 'selected' : '' ?>>Short Answer</option>
                        <option value="long" <?= ($settings['sec_a_type'] ?? '') === 'long' ? 'selected' : '' ?>>Long Essay</option>
                    </select>
                </div>
                <div class="form-group" style="margin: 0;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 2fr 2fr 2fr; gap: 1.5rem; align-items: end; margin-bottom: 1.5rem; background: var(--bg-main); padding: 1.5rem; border-radius: var(--radius-sm); border: 1px solid var(--border-color);">
                ...
                <div class="form-group" style="margin: 0;">
                    ...
                    <select name="sec_b_type" class="form-control" required>
                        <option value="objective" <?= ($settings['sec_b_type'] ?? '') === 'objective' ? 'selected' : '' ?>>Objective</option>
                        <option value="short" <?= ($settings['sec_b_type'] ?? 'short') === 'short' ? 'selected' : '' ?>>Short Answer</option>
                        <option value="long" <?= ($settings['sec_b_type'] ?? '') === 'long' ? 'selected' : '' ?>>Long Essay</option>
                    </select>
                </div>
                <div class="form-group" style="margin: 0;">
                    <label>Default Questions</label>
            ...
            <div style="display: grid; grid-template-columns: 1fr 2fr 2fr 2fr; gap: 1.5rem; align-items: end; margin-bottom: 2rem; background: var(--bg-main); padding: 1.5rem; border-radius: var(--radius-sm); border: 1px solid var(--border-color);">
                ...
                <div class="form-group" style="margin: 0;">
                    ...
                    <select name="sec_c_type" class="form-control" required>
                        <option value="objective" <?= ($settings['sec_c_type'] ?? '') === 'objective' ? 'selected' : '' ?>>Objective</option>
                        <option value="short" <?= ($settings['sec_c_type'] ?? '') === 'short' ? 'selected' : '' ?>>Short Answer</option>
                        <option value="long" <?= ($settings['sec_c_type'] ?? 'long') === 'long' ? 'selected' : '' ?>>Long Essay</option>
                    </select>
                </div>
                <div class="form-group" style="margin: 0;">
                    <label>Default Questions</label>
